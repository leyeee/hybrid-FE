...
const Colors = colors.map(set => {
    ...
    set.colors = set.colors.map(c => {
        ...
        return {
            ...
            .map(item => item.length > 1 ? item[heteronymIdx] : item)
            .join(' ')
        }
    })
    ...
    const fetchPoetry = () => {
        jinrishici.load(
            result => {
                ...
                obj.content = obj.content.replace(/[，|。|！|？|、]/g, ' ').trim().split(' ');
                setPoetry(obj);
...
const useColor = () => {
    ...
    const SelectedColor = JSON.parse(localStorage.getItem('SELECTED_COLOR')) || SelectedColorSet.colors[0];
